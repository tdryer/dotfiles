...
//
// Based on: https://github.com/0xhckr/ghostty-shaders/blob/main/cursor_blaze.glsl
...
// Antialiasing falloff width in pixels.
const float AA_FALLOFF_WIDTH = 2.0;
...
float antialiasing(float distance) {
    return 1. - smoothstep(0., normalize(vec2(AA_FALLOFF_WIDTH, AA_FALLOFF_WIDTH), 0.).x, distance);
}
...
void mainImage(out vec4 fragColor, in vec2 fragCoord) {
    ...
    // Skip trail when unfocused to avoid animation artifacts from large
    // time deltas caused by infrequent deceptive frames.
    if (iFocus <= 0) return;
    ...
    if (isFarEnough) {
        ...
        newColor = mix(newColor, vec4(sRGBToLinear(iCurrentCursorColor.rgb), iCurrentCursorColor.a), antialiasing(sdfTrail));
